...
def main(args):
    ...
    if model_name == "Qwen2-VL-2B-Instruct":
        ...
        model.model.lm_head  = torch.nn.Linear( 
            in_features=new_output_weight.shape[1],  
            out_features=new_output_weight.shape[0],   # 需与词表大小一致
            bias=False
        ).to(model.model.lm_head.weight.device).to(model.model.lm_head.weight.dtype)
    
        # 将拷贝的权重赋给新输出层 
        with torch.no_grad(): 
            model.model.lm_head.weight.copy_(new_output_weight) 
            
    utils.seed_everything(args.seed)
    if not args.not_fuse_layer_norms:
        fuse_qwen2vl_layer_norms(model, args)
    if args.rotate:
        rotate_qwen2vl_model(model.model, args)
        ...
        for name in qlayers:
            if "mlp.fc2" in name:
                had_K, K = hadamard_utils.get_hadK(
                    int(model.model.visual.blocks[0].mlp.fc2.module.in_features)
                )
                qlayers[name].online_full_had = True
                qlayers[name].had_K = had_K
                qlayers[name].K = K
    ...
    if args.max_new_tokens is not None:
        model.generate_kwargs["max_new_tokens"] = args.max_new_tokens
    eval_dataset(
        model,
        dataset,
        args.dataset_name,
